...
<section id="content">
	<div id="board-write-container">
		...
		<table id="tbl-board">
			...
			<tr>
				...
				<td>
					...
					<c:if test="${ not empty board.originalFileName }">
						<a href="javascript:" id="fileDown"> <span> ${ board.originalFileName } </span>
						</a>
						<%-- 	<a href="${ path }/resources/upload/board/${ board.renamedFileName }" download="${ board.originalFileName }"> <span> ${ board.originalFileName } </span>
						</a> --%>
					</c:if>
				</td>
			</tr>
			...
			<%--글작성자/관리자인경우 수정삭제 가능 --%>
			<tr>
				<th colspan="2"><c:if test="${ not empty loginMember && loginMember.id == board.writerId }">
						<button type="button" onclick="location.href='${ path }/board/update?no=${ board.no }'">수정</button>
						<button type="button" id="btnDelete">삭제</button>
					</c:if>
					<button type="button" onclick="javascript:history.back()">목록으로</button></th>
			</tr>
		</table>
		<div id="comment-container">
			<div class="comment-editor">
				<form action="${ path }/board/reply" method="POST">
					...
					<textarea name="content" cols="55" rows="3" id="replyContent"></textarea>
