...
//			cookie.setMaxAge(-1); cf) 브라우저가 종료될 때까지 유지되는 세션 쿠키
		...
		if (loginMember != null) {
			...
			// 로그인이 완료되면 홈화면으로 이동한다.
			response.sendRedirect(request.getContextPath() + "/");

		} else {
			// 로그인 실패에 대한 메시지를 띄우고 홈 화면으로 이동

			// 1. 공용으로 사용하는 에러 메시지 출력 페이지에 전달할 메시지와 메시지 출력 후 이동할 페이지를 request 객체에 저장한다.
			request.setAttribute("msg", "아이디나 비밀번호가 일치하지 않습니다.");
			request.setAttribute("location", "/");

			// 2. request 객체의 데이터를 유지해서 에러 메시지를 출력 페이지에 전달하기 위해 forward를 실행
